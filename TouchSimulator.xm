...
//Internal handle to BackBoardServices
static void *handle = dlopen("/System/Library/PrivateFrameworks/BackBoardServices.framework/BackBoardServices", RTLD_NOW);
...
static BKSHIDEventSetDigitizerInfoType digitizer = (BKSHIDEventSetDigitizerInfoType)dlsym(handle, "BKSHIDEventSetDigitizerInfo");

static IOHIDEventRef createParentEventRef() {
    return IOHIDEventCreateDigitizerEvent(kCFAllocatorDefault, //allocator
...
static void postEvent(IOHIDEventRef event) {
    ...
    if (event != NULL && keyWindow != NULL) {
        ...
        if (handle) {
            //void(* BKSHIDEventSetDigitizerInfoType)(IOHIDEventRef,uint32_t,uint8_t,uint8_t,CFStringRef,CFTimeInterval,float);
